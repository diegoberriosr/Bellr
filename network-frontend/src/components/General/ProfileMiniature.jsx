...
const ProfileMiniature = ({text, textStyle, account}) => {
  ...
  return (
    <div className='flex flex-col items-center justify-center' onMouseLeave={handleVisbility}>
      <span className={textStyle} onMouseEnter={handleVisbility}>{text}</span>
      <div className={`${visible ? 'block' : 'hidden'} fixed top-6 ${mode.background} border rounded-md border-gray-900 w-[250px] flex flex-col justify-center items-start p-2.5 animate-grow`}>
